<div class="mx-auto p-6" wire:key="manage-subject-chiefs-{{ $subject->id }}">

    <div class="mb-6" x-data x-init="$el.classList.add('opacity-100')" class="transition-opacity duration-500 opacity-0">
        <h2 class="text-xl font-semibold text-slate-400">Animateurs d'Etablissement (AE) — {{ $subject->name }}</h2>
        <p class="text-sm text-slate-500 mt-1">Un enseignant ne peut occuper qu'un seul poste (AE principal ou AE
            adjoint).</p>
    </div>
        ...
        <div x-data="{ show: false }" x-init="setTimeout(() => show = true, 50)" x-show="show"
            ...
            <select wire:model.live="principalId"
                ...
                @foreach ($this->availableTeachers as $teacher)
                    <option
                        @if ($teacher->hasCurrentlyCARole() || $teacher->hasCurrentlyPPRole()) title="{{ $teacher->getFullName() }} a déjà au moins un rôle de PP ou de CA" @endif
                        @disabled($teacher->hasCurrentlyCARole() || $teacher->hasCurrentlyPPRole()) class="p-3" value="{{ $teacher->id }}">
    ...
    <div class="flex flex-col gap-4 my-5">
        ...
        @if ($this->principalTeacher)
            <div wire:key="principal-active-{{ $principalId }}" x-data="{ show: false }" x-init="setTimeout(() => show = true, 30)"
                ...
                <span class="text-sm text-slate-200 font-medium">
                    <span class="text-slate-500 mr-3">AE PRINCIPAL</span>
        ...
        @if ($this->adjointTeacher)
            <div wire:key="adjoint-active-{{ $adjointId }}" x-data="{ show: false }" x-init="setTimeout(() => show = true, 30)"
                ...
                <span class="text-sm text-slate-200 font-medium">
                    <span class="text-slate-500 mr-3">AE ADJOINT</span>
    ...
    @if ($principalId || $adjointId)
        <div class="mt-6 flex justify-end">
            <button wire:click="save" wire:loading.attr="disabled" wire:target="save"
                ...
                <span wire:loading.remove wire:target="save">Enregistrer les AE</span>
