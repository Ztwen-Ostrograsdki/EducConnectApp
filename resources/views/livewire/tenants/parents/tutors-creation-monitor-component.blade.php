<div class="w-full mx-auto max-w-7xl border border-slate-700 rounded-3xl p-6 bg-slate-950/50">
    ...
    <section class="bg-slate-900/80 backdrop-blur-2xl rounded-3xl p-6 mb-8 shadow-xl border border-slate-700/50">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div class="flex items-center gap-5">
                <div
                    class="w-16 h-16 rounded-3xl bg-gradient-to-br from-indigo-500/10 to-violet-500/10 
                ...
                <div>
                    ...
                    <p class="text-slate-400 text-sm mt-1">
                        Gestion des migrations des parents ou tuteurs <span class="hidden">{{ $renderKey }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-3 mt-6">
            <a href="{{ route('tenant.parents.create') }}"
                class="p-2 rounded-2xl bg-indigo-600 hover:bg-indigo-500 transition-all active:scale-95 font-medium gap-2 inline-flex items-center">
                <x-lucide-plus class="w-4 h-4" />
                Ajouter des parents / tuteurs
            </a>

            <button wire:click="clearAllBatches" wire:loading.attr="disabled" wire:target="clearAllBatches"
                class="p-2 rounded-2xl bg-orange-600/40 hover:bg-orange-600 text-white transition-all active:scale-95 inline-flex items-center gap-2">
            ...
            <button wire:click="deleteAllSuccess" wire:loading.attr="disabled" wire:target="deleteAllSuccess"
                class="p-2 rounded-2xl bg-red-600/50 hover:bg-red-600 text-white transition-all active:scale-95">
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach ($batches as $batchData)
                <div
                    class="group bg-slate-900 border border-slate-700 rounded-3xl overflow-hidden 
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start">
                            <div class="flex-1">
                                <h2 class="font-bold text-lg text-white">Import parents / tuteurs</h2>
                            ...
                            @if ($batchData['batch']?->finished())
                                <span
                                    class="px-4 py-1.5 bg-green-500/10 text-green-400 text-xs font-medium rounded-2xl border border-green-500/20">
                                    Terminé
                                </span>
                            @else
                                <span
                                    class="px-4 py-1.5 bg-amber-500/10 text-amber-400 text-xs font-medium rounded-2xl border border-amber-500/20">
                                    En cours
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="px-6 pb-6">
                        <div class="flex justify-between text-xs text-slate-400 mb-2">
                            <span>{{ $batchData['batch']?->processedJobs() ?? 0 }} /
                                {{ $batchData['batch']?->totalJobs ?? 0 }}</span>
                            <span class="font-medium">{{ $batchData['batch']?->progress() ?? 0 }}%</span>
                        </div>
                        <div class="w-full bg-slate-800 h-2.5 rounded-full overflow-hidden">
                            <div class="h-full bg-gradient-to-r from-indigo-500 to-violet-500 rounded-full transition-all duration-700 ease-out"
                                style="width: {{ $batchData['batch']?->progress() ?? 0 }}%">
                            </div>
                        </div>
                    </div>
                    ...
                    <div wire:key='actions-card-{{ $batchData['id'] }}'
                        ...
                        <button wire:click="deleteBatch('{{ $batchData['id'] }}')" wire:loading.attr="disabled"
                            ...
                            <span wire:loading.remove wire:target="deleteBatch('{{ $batchData['id'] }}')">Suppr. cette
                                tâche</span>
                    ...
                    <div class="pt-3 flex justify-end">
                        <span class="text-2xs text-slate-500 text-right p-3 font-mono">
                            Tâche créée le {{ __formatDateTime($batchData['last_created']) }}
                        </span>
                    </div>
                </div>
            @endforeach
    ...
    {{-- MODAL --}}
    <div x-data="{ open: @entangle('showBatchModal') }" x-show="open" x-cloak
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex justify-between items-center p-6 border-b border-slate-700">
                ...
                <button wire:click="closeBatchModal"
                    class="h-10 w-10 rounded-2xl bg-slate-800 hover:bg-slate-700 transition-colors flex items-center justify-center">
            ...
            @if ($selectedBatchStats)
                ...
                {{-- Actions --}}
                <div class="px-6 pt-3 pb-6 flex flex-wrap gap-3">
                    @if ($selectedBatchStats['failed'])
                        ...
                        <button wire:click="deleteBatchFailures" wire:loading.attr="disabled"
                            ...
                            class="px-6 py-3 rounded-2xl bg-red-600 hover:bg-red-700 transition-all">
                    ...
                    <button wire:click="deleteBatch" wire:loading.attr="disabled" wire:target="deleteBatch"
                        class="px-6 py-3 rounded-2xl bg-rose-700 hover:bg-rose-800 transition-all">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-4 custom-scroll">
                @foreach ($selectedBatchTasks as $task)
                    <div
                        class="border border-slate-700 bg-slate-950 rounded-3xl p-6 hover:border-slate-600 transition-colors">
                        <div class="flex justify-between items-start">
                            <div>
                            ...
                            @if ($task->status === 'success')
                                <span
                                    class="px-4 py-1 bg-green-500/10 text-green-400 rounded-2xl text-sm font-medium">Succès</span>
                            @elseif ($task->status === 'failed')
                                <span
                                    class="px-4 py-1 bg-red-500/10 text-red-400 rounded-2xl text-sm font-medium">Échec</span>
                            @else
                                <span
                                    class="px-4 py-1 bg-amber-500/10 text-amber-400 rounded-2xl text-sm font-medium">En
                                    cours</span>
                            @endif
                        </div>
                        ...
                        <div class="flex gap-3 mt-6">
                            @if ($task->status === 'failed')
                                <button wire:click="retryOne({{ $task->id }})" wire:loading.attr="disabled"
                                    wire:target="retryOne({{ $task->id }})"
                                    class="px-5 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-black text-sm font-medium transition-all">
                                    <span wire:loading.remove
                                        wire:target="retryOne({{ $task->id }})">Relancer</span>
                            ...
                            <button wire:click="deleteOne({{ $task->id }})" wire:loading.attr="disabled"
                                wire:target="deleteOne({{ $task->id }})"
                                class="px-5 py-2.5 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white text-sm font-medium transition-all">
                                <span wire:loading.remove
                                    wire:target="deleteOne({{ $task->id }})">Supprimer</span>
                        ...
                        @if ($task->error)
                            <div
                                class="mt-4 text-red-400 text-sm bg-red-950/50 border border-red-900/50 p-4 rounded-2xl">
                                {{ $task->error }}
                            </div>
                        @endif
                    </div>
