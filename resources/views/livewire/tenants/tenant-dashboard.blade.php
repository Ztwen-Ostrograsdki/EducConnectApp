<div>
        ...
        <div>
            <h1 class="font-serif text-3xl italic font-normal leading-tight text-slate-100">
                ...
                <span class="bg-gradient-to-br from-indigo-500 to-emerald-500 bg-clip-text text-transparent">
                    {{ Auth::guard('tenant')->user()?->name ?? 'Directeur' }}
                </span>
                👋
            </h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4 mb-7">
        ...
        <div data-z-card class="bg-slate-800 border transition-[transform] duration-200 hover:-translate-y-0.5 border-slate-700 rounded-2xl p-5">
            ...
            <div
                ...
                class="flex items-end gap-2 h-28 pb-2"
            >
                @foreach([['L',92],['M',88],['Me',95],['J',90],['V',87],['S',82],['D',null]] as $d)
                    ...
                    <div class="flex-1 flex flex-col items-center gap-1">
                        ...
                        @if($d[1] !== null)
                            ...
                            <div
                                class="w-full rounded-t bg-indigo-500/70 transition-all duration-1000 ease-out"
    ...
    >
        ...
        @foreach([
            ['Primaire',  '312', 37, 'bg-emerald-500/70'],
            ['Secondaire','428', 51, 'bg-indigo-500/70'],
            ['Supérieur', '107', 12, 'bg-amber-500/70'],
        ] as $n)

        <div
            class="mb-3.5 transition-all duration-700 ease-out"
            :style="show
        ...
        <div
            class="mt-4 pt-3.5 border-t border-slate-700
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-7">
        ...
        <div data-z-card class="bg-slate-800 border border-slate-700 transition-[transform] duration-200 hover:-translate-y-0.5 rounded-2xl overflow-hidden">
            ...
            <div class="py-1">
                @foreach([
                    ['📝','Notes saisies — Maths 3ème 1','Prof. Amoussou','5 min'],
                    ['👤','Nouvel apprenant inscrit','Kofi Danso','12 min'],
                    ['💳','Paiement reçu — 25 000 FCFA','Famille Mensah','1h'],
                    ['🔔','Absence signalée — Terminale C','Auto','2h'],
                    ['📄','Bulletin généré — Tle C','Directeur','3h'],
                    ['👩‍🏫','Enseignant affilié — Prof. Bossou','Directeur','5h'],
                    ['⚠️','Note modifiée avec justification','Prof. Koudé','7h'],
                ] as $a)
