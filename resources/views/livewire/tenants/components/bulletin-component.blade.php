            ...
            <div class="flex flex-wrap items-center gap-2 shrink-0 justify-end">
                @if (!$this->hasCurrentBulletin($student->id))
                    <button wire:click='generateStudentBulletin({{ $student->id }})'
                        class="py-3 px-3 rounded-2xl bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20 text-orange-300 text-xs font-medium transition-all inline-flex items-center justify-center">
                        <span class="inline-flex items-center">
                            <span class="inline-flex items-center gap-x-2">
                                <x-lucide-blocks class="w-4 h-4" />
                                <span>
                                    Générer bulletin @if ($this->period)
                                        du {{ $this->activeYear->periodLabel() . ' ' . $this->period }}
                                    @endif
                                </span>
                            </span>
                        </span>
                ...
                @else
                    <button wire:click='generateStudentBulletin({{ $student->id }})'
                        class="py-3 px-3 rounded-2xl bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 text-indigo-300 text-xs font-medium transition-all inline-flex items-center justify-center">
                        <span class="inline-flex items-center">
                            <span class="inline-flex items-center gap-x-2">
                                <span wire:loading.remove wire:target="generateStudentBulletin({{ $student->id }})">
                                    <span class="inline-flex items-center gap-x-2">
                                        <x-lucide-database-backup class="w-4 h-4" />
                                        <span>
                                            Mettre à jour bulletin @if ($this->period)
                                                du {{ $this->activeYear->periodLabel() . ' ' . $this->period }}
                                            @endif
                                        </span>
                                    </span>
                                </span>
                                <span wire:loading.flex wire:target="generateStudentBulletin({{ $student->id }})"
                    ...
                    <button wire:click='printStudentBulletin({{ $student->id }})'
                        class="py-3 px-3 rounded-2xl bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 text-purple-400 text-xs font-medium transition-all inline-flex items-center justify-center">
                        <span class="inline-flex items-center">
                            <span class="inline-flex items-center gap-x-2">
                                <span wire:loading.remove wire:target="printStudentBulletin({{ $student->id }})">
                                    <span class="inline-flex items-center gap-x-2">
                                        <x-lucide-download class="w-4 h-4" />
                                        <span>
                                            Télécharger bulletin @if ($this->period)
                                                du {{ $this->activeYear->periodLabel() . ' ' . $this->period }}
                                            @endif
                                        </span>
                                    </span>
                                </span>
                                <span wire:loading.flex wire:target="printStudentBulletin({{ $student->id }})"
                                    class="relative items-center gap-2 text-rose-300">
                                    <x-lucide-loader-2 class="w-4 h-4 animate-spin" />
                                </span>
                            </span>
    ...
    <div class="mx-auto w-full max-w-[1400px] mb-40">
        ...
        <div class="border border-white/[0.06] bg-[#0f1523] shadow-xs shadow-purple-700 overflow-hidden">
            ...
            @if ($classe && $this->effectifs)
                                ...
                                <div class="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
                                    @foreach ([['Matricule', $student->matricule], ['Classe', $classe?->code], ['Sexe', $student->gender], ['Né(e) le', formatBirthDate($this->student->birth_date)], ['Nationalité', $student->country], ['Effectif', $this->effectifs['apprenants']], ['Prof. Principal', $classe?->principal?->getFullName() ?? '—'], ['Contact école', tenant('contacts')]] as $info)
                                        <div class="rounded-xl bg-[#070b14] border border-white/[0.05] px-3.5 py-3">
                                            <p
                                                class="text-[10px] uppercase tracking-wider text-slate-600 font-semibold">
                                                {{ $info[0] }}</p>
                                ...
                                <table class="min-w-[1100px] w-full text-sm">
                                                ...
                                                <td class="px-4 py-3.5">
                                                    @if (!empty($row['mention']))
                                                        ...
                                                        {{ str_contains(strtolower($row['mention']), 'bien') || str_contains(strtolower($row['mention']), 'très')
                                                            ...
                                                            : (str_contains(strtolower($row['mention']), '...')
                                    ...
                                    <tfoot>
                                        <tr class="bg-[#070b14] border-t border-white/[0.08]">
                                            ...
                                            <td
                                                class="px-3 py-4 text-center text-xl font-bold text-indigo-400 font-mono">
                                            ...
                                            <td
                                                class="px-3 py-4 text-center font-bold text-emerald-400 font-mono text-xl">
                                            ...
                                            <td colspan="2" class="px-4 py-4">
                                                <span
                                                    class="inline-flex px-3 py-1 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-lg font-semibold">
                ...
                @endif
            @else
                <span class="bg-transparent text-slate-600 flex items-center justify-center w-full p-4 my-4">
                    Cet apprenant est soit sans classe soit sa classe est vide
                </span>
            @endif
