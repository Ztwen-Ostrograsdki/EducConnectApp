    ...
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold">Migration des apprenants
                    @if ($selectedClasse)
                        vers la classe <span class="text-green-600 font-mono">{{ $selectedClasse->name }}</span>
                    @else
                        <span class="text-orange-600 font-mono text-xs ml-2.5 animate-pulse">Veuillez sélectionner une
                            classe de
                            destination</span>
                    @endif
                </h1>
                <p class="mt-1 text-sm text-slate-400">
                    ...
                    @if ($selectedClasse)
                        à la classe <span class="text-green-600 font-mono">{{ $selectedClasse->name }}</span> pour le
                        compte de l'année scolaire
                    @else
                        à une classe ·
                    @endif
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            ...
            <select wire:model.live="classeId"
                ...
                <option @if (!empty($draftStudents)) disabled @endif value="">— Sélectionner une classe —
                </option>
        ...
        @if ($classeId && $selectedClasse)
            ...
            @if ($mode === 'excel')
                <div class="rounded-2xl border border-slate-800 bg-slate-900 p-5 space-y-5">
                    ...
                    <div
                        ...
                        <p>· <span class="font-mono text-amber-400">identifiant / id / code</span> → recherche sur les
                            deux
                    ...
                    @if ($browseStudents)
                        ...
                        @if (count($draftStudents) > 0)
                            <div class="flex items-center justify-between text-xs text-slate-500">
                                ...
                                <span class="text-indigo-400 font-medium">{{ count($draftStudents) }}
                                    sélectionné(s)</span>
                            </div>
                        @endif
                        ...
                        <div class="space-y-1 max-h-[500px] overflow-y-auto pr-1" wire:loading.class="opacity-50"
                            ...
                            @forelse ($browseStudents as $student)
                                ...
                                @php $draftEntry = $inDraft ? collect($draftStudents)->firstWhere('id', $student->id) : null; @endphp
            ...
            @endif
        @else
            <h5 class="animate-pulse font-bold text-2xl text-center w-full py-10 text-slate-600">Veuillez
        ...
        @if (!empty($draftStudents))
            <div class="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
                ...
                <div class="px-5 py-4 border-b border-slate-800 flex items-center justify-between gap-4">
                    ...
                    <button wire:click="clearDraft" wire:loading.attr="disabled" wire:target="clearDraft"
                        class="relative px-6 py-2 rounded-xl border border-rose-500/30 bg-rose-500/10 text-rose-400 text-sm hover:bg-rose-500/20 transition disabled:opacity-50">
                        <span wire:loading.remove wire:target="clearDraft">Vider la sélection</span>
                        <span wire:loading wire:target="clearDraft" class="flex items-center gap-2">
                            <svg class="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10"
                ...
                {{-- Liste draft --}}
                <div class="divide-y divide-slate-800 max-h-96 overflow-y-auto font-mono">
