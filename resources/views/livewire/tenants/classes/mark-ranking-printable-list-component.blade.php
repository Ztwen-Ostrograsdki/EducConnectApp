<div class="print-wrapper">
    ...
    <header class="doc-header">
        ...
        <div class="doc-counter">
            <h4>Total: {{ __zero($allStudents) }} Apprenant(s)</h4>
        </div>
    </header>
    ...
    <main class="doc-body">
        ...
        @else
            @php
                ...
                $widths = \App\Services\MarksServices\MarksPrintFormatter::normalizedWidths($columns);
            @endphp
                    ...
                    @foreach ($rows as $row)
                        @if ($row['groupStart'])
                            <tr class="group-header-row">
                                <td colspan="{{ count($columns) + 1 }}">{{ $row['groupLabel'] }}</td>
                            </tr>
                        @endif
                        <tr class="{{ $loop->even ? 'row-even' : 'row-odd' }}">
                            ...
                            @foreach ($columns as $col)
                                <td
                                    class="{{ $col['key'] === 'student' ? 'cell-name' : 'td-center' }}
                                    {{ $col['type'] === 'mark' ? 'cell-numeric' : '' }}">
                                    {!! $row['cells'][$col['key']] ?? '—' !!}
                                </td>
                            @endforeach
                        </tr>
