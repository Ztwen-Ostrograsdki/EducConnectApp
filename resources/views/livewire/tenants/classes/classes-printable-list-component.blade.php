        ...
        <div class="header-band">
            ...
            <div class="doc-stamp">
                ...
                <span class="stamp-ref">
                    Réf : PERS-CLS-{{ now()->format('Ymd') }}-{{ str_pad($allClasses, 3, '0', STR_PAD_LEFT) }}
                </span>
            </div>
        </div>
    ...
    <main class="doc-body">
        @if (empty($rows))
            <div class="empty-state">
                <p>Aucune classe à afficher pour les critères sélectionnés.</p>
            </div>
        @else
            @php
                $columns = $tableColumns;
                $widths = \App\Livewire\Tenants\Classes\ClassesPrintableListComponent::normalizedWidths($columns);
                $numericColumns = [
                    'effectif_total',
                    'effectif_garcons',
                    'effectif_filles',
                ...
                <tbody>
                    @foreach ($rows as $row)
                        <tr class="{{ $loop->even ? 'row-even' : 'row-odd' }}">
                            ...
                            @foreach ($columns as $col)
                                <td
                                    class="{{ $col['key'] === 'name' ? 'cell-name' : 'td-center' }}
                                    {{ in_array($col['key'], $numericColumns) ? 'cell-numeric' : '' }}">
                                    @if ($col['key'] === 'name')
                                        <span class="name-main">{!! $row['cells']['name'] !!}</span>
                                    @else
                                        {!! $row['cells'][$col['key']] ?? '—' !!}
                                    @endif
                                </td>
