<div class="mx-auto w-full space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Modifier la série</h1>
            <p class="text-sm text-slate-500 mt-0.5">{{ $serial->name }}</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-700 bg-slate-900 p-6 space-y-5">
        <div>
            ...
            <input type="text" wire:model.live="name" placeholder="ex: Série A, Série C"
        ...
        <div>
            ...
            <input type="text" wire:model="code" placeholder="ex: A, B, C"
                class="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none transition" />
            @error('code')
                <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
            @enderror
        ...
        <div>
            <label class="block text-xs font-medium text-slate-400 mb-1.5">Description <span
                    class="text-slate-600">(optionnel)</span></label>
            <textarea wire:model="description" rows="3" placeholder="Description de la série..."
        ...
        <label class="flex items-center gap-3 cursor-pointer">
            ...
            <span class="text-sm text-slate-300">Série active</span>
        </label>
    </div>
    ...
    <div class="flex justify-end gap-3">
        ...
        <button wire:click="save" wire:loading.attr="disabled"
            ...
            <span wire:loading.remove wire:target="save"><x-lucide-check class="w-4 h-4 inline -mt-0.5" />
                Enregistrer</span>
