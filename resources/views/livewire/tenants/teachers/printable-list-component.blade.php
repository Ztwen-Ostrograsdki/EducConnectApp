...
<div class="print-wrapper">
    ...
    <header class="doc-header">
        ...
        <div class="header-band">
            ...
            <div class="doc-stamp">
                ...
                <span class="stamp-ref">Réf : PERS-ENS-{{ now()->format('Ymd') }}-{{ str_pad($allTeachers, 3, '0', STR_PAD_LEFT) }}</span>
            </div>
        </div>
    ...
    <main class="doc-body">
        ...
        @else
            <table class="teachers-table text-center">
                <thead>
                    <tr>
                        <th class="col-num">#</th>
                ...
                <tbody>
                    @foreach ($teachers as $index => $teacher)
                        <tr class="{{ $loop->even ? 'row-even' : 'row-odd' }}">
                            ...
                            <td class="col-dept">
                                <p class="dept-main">{{ $teacher->user->city ?? '—' }}</p>
                                <p class="dept-matiere">{{ '-' }}</p>
                            </td>

                            <td class="col-grade">{{ $teacher->user->grade ?? '—' }}</td>

                            <td class="col-contact">
                                @if ($teacher->user->contacts)
                                    <p class="contact-line">{{ $teacher->user->contacts }}</p>
                                @endif
                                @if ($teacher->user->email)
                                    <p class="contact-line contact-email">{{ $teacher->user->email }}</p>
                                @endif
                            </td>
...
<style>
    ...
    *::after {
    ...
    .teachers-table thead th.td-center {
        text-align: center;
    }

    /* Lignes alternées */
    .teachers-table tbody tr.row-odd {
        background: var(--white);
    }

    .teachers-table tbody tr.row-even {
        background: var(--slate-mid);
    }
