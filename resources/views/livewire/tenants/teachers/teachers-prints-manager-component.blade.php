<div class="min-h-screen bg-[#070b14] text-slate-100" x-data="{ showColumnsHint: false }">
    <div class="mx-auto max-w-[1100px] px-4 sm:px-6 py-10 relative">

        {{-- Loading overlay --}}
        <div wire:loading wire:target="resetFilters, restoreSelects"
            class="fixed inset-0 z-[200] flex items-center justify-center bg-[#070b14]/80 backdrop-blur-sm">
            <div class="flex flex-col items-center gap-4">
                <div class="w-14 h-14 rounded-full border-2 border-sky-500/30 border-t-sky-400 animate-spin"></div>
        ...
        <header class="mb-10 text-center">
            <div
                class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-300 text-[11px] font-semibold uppercase tracking-[0.2em] mb-4">
                <span class="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse"></span>
                Impression des enseignants
            </div>
        ...
        <div class="space-y-6">
            ...
            <section
                ...
                <div class="px-6 py-4 border-b border-white/[0.06] flex items-center gap-3">
                    <div
                        class="flex items-center justify-center w-9 h-9 rounded-xl bg-sky-500/15 border border-sky-500/25">
                        <span class="text-lg">🎯</span>
                    </div>
                    <div>
                        <h2 class="text-sm font-semibold text-white">Cibler les enseignants</h2>
                        <p class="text-[11px] text-slate-500">Classe, matière, filière, localisation…</p>
                    </div>
                </div>
                    ...
                    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        @if (!$filiar_id && !$serial_id && !$promotion_id && !$promotionInGroups)
                            <select @disabled($classe_slug && $classe_id) wire:model.live="classe_id"
                                class="h-11 rounded-xl bg-[#070b14] border border-white/10 px-3 text-xs text-slate-300 focus:outline-none focus:border-sky-500/40 transition-all disabled:opacity-40">
                            ...
                            @if (!$promotion_id)
                                @if (!$serial_id)
                                    <select wire:model.live="filiar_id"
                                        class="h-11 rounded-xl bg-[#070b14] border border-white/10 px-3 text-xs text-slate-300 focus:outline-none focus:border-sky-500/40 transition-all">
                        ...
                        <select wire:model.live="department"
                            ...
                            @foreach ($this->departments as $dpv)
                ...
                <div class="p-6">
                    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <select wire:model.live="trashedStatus"
                            ...
                            @foreach ($trashedStatuses as $key => $label)
                                <option value="{{ $key }}">{{ $label }}</option>
                            @endforeach
                        </select>

                        <select wire:model.live="accessStatus"
                            class="h-11 rounded-xl bg-[#070b14] border border-white/10 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/40 transition-all">
                            @foreach ($accessStatuses as $key => $label)
                                <option value="{{ $key }}">{{ $label }}</option>
                            @endforeach
                        </select>

                        @if (!$classe_id)
                            <select wire:model.live="hasClassesStatus"
                                class="h-11 rounded-xl bg-[#070b14] border border-white/10 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/40 transition-all">
                                @foreach ($hasClassesStatuses as $key => $label)
                                    <option value="{{ $key }}">{{ $label }}</option>
                                @endforeach
                            </select>
                        @endif
                        ...
                        <select wire:model.live="aeStatus"
                            class="h-11 rounded-xl bg-[#070b14] border border-white/10 px-3 text-xs text-slate-300 focus:outline-none focus:border-emerald-500/40 transition-all">
            ...
            <section
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-center gap-3">
                        <span
                            class="inline-flex items-center px-3 py-1.5 rounded-full bg-sky-500/15 border border-sky-500/25 text-sky-300 text-sm font-semibold tabular-nums">
                            {{ __zero($this->allTeachersCounter) }}
                        </span>
                        <span
                            class="text-sm text-slate-400">enregistrement{{ $this->allTeachersCounter > 1 ? 's' : '' }}
                            trouvé{{ $this->allTeachersCounter > 1 ? 's' : '' }}</span>
                    </div>
            ...
            <section
                ...
                <div class="p-6 space-y-3">
                                ...
                                <label class="flex items-center justify-between gap-2 cursor-pointer">
                                    <span class="flex items-center gap-2 min-w-0">
                                        <template x-if="checked">
                                            <span
                                                class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-sky-600 text-[10px] font-bold text-white">
                                                {{ $order !== false ? $order + 1 : '' }}
                                            </span>
                                        </template>
                                        <span
                                    ...
                                    <span class="relative flex shrink-0">
                                        ...
                                        <span
                                            class="h-5 w-9 rounded-full bg-slate-700 transition-colors duration-200 peer-checked:bg-sky-600"></span>
                                        <span
                    ...
                    <p class="text-center text-slate-600 text-xs font-mono pt-2">
                        Cochez une colonne pour personnaliser l’entête…
                    </p>
                </div>
            </section>
                ...
                <div class="px-6 py-4 border-b border-white/[0.06]">
                    <h2 class="text-sm font-semibold text-white">
                        @if (count($this->orderedColumns))
                            Aperçu de l’entête
                            <span class="text-sky-400 font-normal text-xs ml-1">(personnalisé)</span>
                        @else
                            Aperçu de l’entête
                            ...
                            <thead>
                                <tr class="border-b border-white/10">
                                    ...
                                    @foreach ($this->orderedColumns as $th)
                                        <th
                                            class="px-4 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-sky-300/80 whitespace-nowrap">
                                            {{ $th }}
                                        </th>
                                    @endforeach
                                </tr>
            ...
            <div class="pt-4 pb-16">
                <button wire:click="initPrintProcess" wire:loading.attr="disabled" wire:target="initPrintProcess"
                    ...
                    <span
                        class="absolute inset-0 bg-gradient-to-r from-sky-600/80 via-indigo-600/80 to-violet-600/80 opacity-90 group-hover:opacity-100 transition-opacity duration-300"></span>

                    {{-- Glow animé --}}
                    <span
                        class="absolute -inset-1 bg-gradient-to-r from-sky-500 via-indigo-500 to-violet-500 rounded-2xl blur-xl opacity-30 group-hover:opacity-60 transition-opacity duration-500 -z-10"></span>
                        ...
                        <span wire:loading.remove wire:target="initPrintProcess"
                            ...
                            <span class="flex flex-col items-start sm:items-center text-left sm:text-center">
                                ...
                                <span class="text-white/60 text-[11px] font-medium mt-0.5">
                                    {{ __zero($this->allTeachersCounter) }}
                                    enregistrement{{ $this->allTeachersCounter > 1 ? 's' : '' }} seront imprimés
                                </span>
                            </span>
                        </span>
